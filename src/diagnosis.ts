...
const NAK_MEANINGS: Record<string, string> = {
  ...
  'NO_INTERFACE': 'no radio interface available to transmit',
  'MAX_RETRANSMIT': '3 retransmissions exhausted with no response',
  'NO_CHANNEL': 'requested channel is disabled or unavailable',
  'TOO_LARGE': 'payload exceeds max size for current modem preset',
  'NO_RESPONSE': 'destination reached but no service wants to reply',
  'DUTY_CYCLE_LIMIT': 'transmit suppressed by regional duty-cycle regulation',
};
...
export function renderDiagnosis(s: DeviceSummary): string {
  ...
  if (s.fsOrphan && s.missingCriticalPrefs.length > 0) {
    diag.push({title: 'Config-loss loop: storage writes interrupted by reboots',
      detail: 'LittleFS orphan + missing ' + s.missingCriticalPrefs.join(', ') + '. ' +
  ...
  if (utilNum >= 65) {
    diag.push({title: `Mesh channel saturated (${s.channelUtil}%) — LongFast reliability threshold exceeded`,
      detail: 'At ≥65% utilisation the LongFast preset (and similar wide-BW presets) ' +
  ...
  if (s.espPanicBacktrace) {
    crit.push({title: 'ESP32 crash — panic backtrace captured',
      detail: (s.panicPc ? `First PC: ${s.panicPc}. ` : '') +
  ...
  if (s.criticalErrors.length > 0) {
    ...
    const decoded = s.criticalErrors.slice(0, 3)
        .map((e) => `${critFmt(e.code)} @ ${e.file}:${e.line}`).join('; ');
    crit.push({title: `Critical firmware error${n > 1 ? 's' : ''}: ${critFmt(s.criticalErrors[0].code)}`,
      detail: decoded + (n > 3 ? ` +${n - 3} more` : '')});
  }
  if (s.filesystemMountFailed) {
    crit.push({title: 'Filesystem mount failed — all settings unavailable',
      detail: 'LittleFS could not be mounted. The device runs on factory defaults and ' +
  ...
  if (s.storageDecryptCorrupt) {
    crit.push({title: 'Encrypted storage corrupted — config and keys unloadable',
      detail: 'Decrypt failed and storage was treated as corrupt. ' +
  ...
  if (s.missingCriticalPrefs.length > 0) {
    crit.push({title: 'Critical config files missing — factory defaults loaded',
      detail: `Missing on this boot: ${s.missingCriticalPrefs.join(', ')}. ` +
  ...
  if (s.firmware) {
    ...
    if (tag) {
      warn.push({title: `Running ${tag} pre-release firmware (${s.firmware})`,
        detail: 'Pre-release builds may have known bugs, unfinished features, or changed ' +
          'wire-format that makes them incompatible with older firmware on the same mesh.'});
    }
  }
  if (s.region === 'UNSET') {
    warn.push({title: 'LoRa region not configured — transmit disabled',
      detail: 'The device rejects all outgoing LoRa packets until a region is set. ' +
  ...
  if (rxTotal >= 20 && (s.rxBad ?? 0) / rxTotal >= 0.10) {
    ...
    warn.push({title: `High packet error rate (${pct}% bad of ${rxTotal} received)`,
      detail: 'Packets are received at the radio layer but fail to decode. ' +
  ...
  if (utilNum >= 50 && utilNum < 65) {
    warn.push({title: `Channel utilisation high (${s.channelUtil}%)`,
      detail: 'At 50%+ collisions become frequent. ' +
  ...
  if (s.dutyCycleHits > 0) {
    warn.push({title: `Hit regulatory duty-cycle limit (${s.dutyCycleHits}×)`,
      detail: 'Transmissions were suppressed to comply with regional duty-cycle rules. ' +
  ...
  if (!gpsDisabled) {
    if (s.gpsChipModel && s.gpsLocksAcquired === 0 && s.gpsSearchFailures > 0 &&
      ...
      warn.push({title: `GPS hardware (${s.gpsChipModel}) detected but never obtained a fix`,
        detail: `${s.gpsSearchFailures} failed search${s.gpsSearchFailures !== 1 ? 'es' : ''}. ` + hint});
    }
    if (s.gpsBufferFullEvents > 0) {
      warn.push({title: `GPS UART buffer overrun (${s.gpsBufferFullEvents}×)`,
        detail: `GPS serial buffer had ${s.gpsBufferFullBytes ?? 'unknown'} bytes waiting when it overflowed. ` +
    ...
    if (s.gpsConfigNacks > 5 || s.gpsConfigTimeouts > 5 || s.gpsAtgmConfigFailures > 0) {
      ...
      warn.push({title: `GPS module rejecting config commands ` +
          `(${s.gpsConfigNacks} NACKs, ${s.gpsConfigTimeouts} timeouts)`,
      detail: (msgs ? `UBX messages: ${msgs}. ` : '') +
    ...
    if ((s.gpsConsecutiveLockFailures ?? 0) >= 3 && s.gpsLocksAcquired > 0) {
      warn.push({title: `GPS lost lock — ${s.gpsConsecutiveLockFailures} consecutive failures since last fix`,
        detail: 'GPS was working earlier this session but re-acquisition is failing now. ' +
  ...
  if (s.watchdogReset || s.espRstCode === 'RTCWDT_RTC_RESET' || s.espRstCode === 'TG1WDT_SYS_RESET') {
    warn.push({title: 'Previous reboot was a watchdog reset',
      detail: (s.espRstCode ? `ESP reset code: ${s.espRstCode}. ` : '') +
  ...
  if (s.retransmissions > 20) {
    warn.push({title: `High retransmission count (${s.retransmissions})`,
      detail: 'ACK-requested packets are not being acknowledged on the first attempt. ' +
  ...
  if (s.fsOrphan && s.missingCriticalPrefs.length === 0) {
    warn.push({title: 'LittleFS orphan found — filesystem inconsistency recovered',
      detail: 'A partial write was left as an orphaned inode and recovered at mount time. ' +
  ...
  if (s.radioLibErrors.length > 0) {
    ...
    const decoded = s.radioLibErrors.slice(0, 3)
        .map((e) => `${e.radio} ${e.op} → ${rlibFmt(e.code)}`).join('; ');
    warn.push({title: `RadioLib errors (${n} event${n !== 1 ? 's' : ''})`,
      detail: decoded + (n > 3 ? ` +${n - 3} more. ` : '. ') +
        (hasSpiErrors(s) ?
  ...
  if (s.radioRecoveryReboot && !hasSpiErrors(s)) {
    warn.push({title: 'Radio entered recovery reboot cycle',
      detail: 'Repeated radio failures triggered an automatic device reboot. ' +
  ...
  if (s.warmstoreRingCorrupt || s.deviceStateDiscarded) {
    warn.push({title: s.warmstoreRingCorrupt ?
  ...
  if (s.lockdownActive) {
    warn.push({title: 'Storage lockdown active — admin commands rejected',
      detail: (s.lockdownState ? `State: ${s.lockdownState}. ` : '') +
  ...
  if (s.busyRxCount > 20) {
    warn.push({title: `Frequent RX-busy collisions (${s.busyRxCount})`,
      detail: 'The radio was still in RX when a TX was requested. ' +
  ...
  if (s.channelDecodeFailures > 10) {
    warn.push({title: `Repeated channel decode failures (${s.channelDecodeFailures})`,
      detail: 'Packets pass radio-layer CRC but fail decryption or protobuf decode. ' +
  ...
  if (totalNaks > 30) {
    ...
    warn.push({title: `Excessive NAK errors (${totalNaks} total)`,
      detail: (top ?
        `Most frequent: ${top[0]} × ${top[1]} — ${meaning}. ` : '') +
        ...
        'NO_CHANNEL = channel disabled.'});
  }
  if (s.safeFileWriteFailures > 5) {
    warn.push({title: `Repeated safe-file write failures (${s.safeFileWriteFailures})`,
      detail: 'SafeFile atomic writes are failing: tmp file rename fails or readback hash mismatches. ' +
  ...
  if (s.fromRadioQOverflow > 0) {
    warn.push({title: `Radio→app queue overflow (${s.fromRadioQOverflow}×)`,
      detail: 'Packets are delivered from radio to the app layer faster than they can be consumed. ' +
  ...
  if (s.nvsUsed !== undefined && s.nvsFree !== undefined && (s.nvsUsed + s.nvsFree) > 0) {
    ...
    if (s.nvsUsed / nvsTotal > 0.85) {
      warn.push({title: `NVS flash nearly full (${s.nvsUsed}/${nvsTotal} entries, ` +
          `${Math.round((s.nvsUsed / nvsTotal) * 100)}%)`,
      detail: 'NVS (non-volatile storage) holds BLE bonding keys, some config, and OTA state. ' +
  ...
  if (gpsDisabled) {
    info.push({title: 'GPS disabled by user — position and time-sync inactive',
      detail: 'gpsUserMode = DISABLED. No lock attempts are made; GPS-related warnings above are not applicable. ' +
        'Re-enable GPS in device settings to restore position and NTP-like time correction.'});
  }
  if (s.gpsDefaultsMaintained && !gpsDisabled) {
    info.push({title: 'GPS running on factory defaults (GPS-only, no GLONASS/Galileo)',
      detail: 'The chip rejected the firmware\'s multi-constellation configuration. ' +
  ...
  if (utilNum >= 25 && utilNum < 50) {
    info.push({title: `Channel utilisation elevated (${s.channelUtil}%)`,
      detail: 'Above 25% client nodes automatically suppress their own telemetry broadcasts. ' +
  ...
  if (s.buildVariant && s.buildVariant !== 'meshtastic/firmware') {
    const hasStorageIssues = s.fsOrphan || s.safeFileWriteFailures > 0 ||
      s.configSaveFailures > 0 || s.adminConfigSaveFailed;
    info.push({title: `Custom firmware fork: ${s.buildVariant}`,
      detail: (hasStorageIssues ?
  ...
  if (s.tcxoFallbackToXtal) {
    info.push({title: 'Radio oscillator in XTAL mode (TCXO absent or failed)',
      detail: 'The LR11x0/LR20x0 fell back to internal crystal after TCXO init failed. ' +
  ...
  if (s.batteryPct === '101') {
    info.push({title: 'Device is externally powered (USB or DC) — no battery measurement',
      detail: 'battery_level=101 is the Meshtastic convention for "externally powered / no battery". ' +
  ...
  if (s.nodeDbFullEvictions > 5) {
    info.push({title: `Node database full — oldest nodes evicted ${s.nodeDbFullEvictions} times`,
      detail: `The default node DB limit is 100 nodes (80 on T-Echo). ` +
  ...
  if (s.factoryReset) {
    info.push({title: 'Factory reset performed during this session',
      detail: 'All settings cleared. Admin payload 94 triggers a full factory reset. ' +
  ...
  if (s.sensorsDropped.length > 0) {
    ...
    info.push({title: `I²C sensor${n > 1 ? 's' : ''} detected on bus but dropped by driver`,
      detail: `${s.sensorsDropped.join(', ')}. ` +
  ...
  if (s.storeForwardPsramFull) {
    info.push({title: 'Store & Forward PSRAM buffer full — oldest messages overwriting',
      detail: 'The S&F ring buffer has wrapped. Late-connecting clients miss earlier messages. ' +
        'Reduce the S&F record count or retention period in module settings.'});
  }
  if (s.securityWarning) {
    info.push({title: 'Public key broadcast triggered',
      detail: '"advertised your public key" logged: the device broadcast its public key ' +
  ...
  if (s.positionZeroSkipped > 0 && !s.gpsLock && !gpsDisabled) {
    info.push({title: `Position broadcasts suppressed — GPS has no fix yet (${s.positionZeroSkipped}×)`,
      detail: 'Zero-coordinate packets are filtered to prevent poisoning the mesh with ' +
  ...
  if (s.configCoerced) {
    info.push({title: 'Telemetry/position intervals auto-coerced to role minimum',
      detail: 'The firmware detected a configured interval shorter than the role-aware minimum and adjusted it. ' +
  ...
  if (s.reliableSendFailures > 10) {
    info.push({title: `Reliable send failures (${s.reliableSendFailures})`,
      detail: 'Messages sent with ACK request received no reply after 3 retransmissions. ' +
  ...
  if (s.noHwRng) {
    info.push({title: 'No hardware random number generator (HWRNG)',
      detail: 'The crypto stack uses a software PRNG seeded from timing jitter. ' +
